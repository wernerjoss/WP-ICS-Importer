...
if (! class_exists('icsCalDisplay')) {
	class icsCalDisplay {
		...
		static function initCalendar($options, $eventid='', $eventdate='', $cuid='') {
			$cellWidth = '15%';	// moved here from outer space :) 15.11.18
			$cellHeight = '50px';
			if(empty($cuid)) $cuid = rand(1,5000);
			// $cellStyle = ' style="width:'.$this->cellWidth.'; height:'.$this->cellHeight.'; "';
			$cellStyle = ' style="width:'.$cellWidth.'; height:'.$cellHeight.'; "';	// php 7.2 fix 14.11.18
			//$calContent .= _r($calEvents);
			$calContent = '';	// define as empty string first 15.11.18
			...
			$calContent .= '</script>';
			//	$calContent .= '<div class="ics-calendar-header"><small><a class="ics-nav-button" id="ics-prev-button'.$cuid.'" href="javascript:void(0);">&laquo; '.__('VorMonat','WPICSImporter').'</a></small> ' .
							//	'<div id="ics-calendar-header-text'.$cuid.'"></div> <small><a class="ics-nav-button" id="ics-next-button'.$cuid.'" href="javascript:void(0);">'.__('N&auml;chster Monat','WPICSImporter').' &raquo;</a></small></div>';
			$calContent .= '<div class="ics-calendar-header"><a class="ics-nav-button" id="ics-prev-button'.$cuid.'" href="javascript:void(0);">&laquo; '.__(' ','WPICSImporter').'</a> ' . 
							'<div id="ics-calendar-header-text'.$cuid.'"></div> <a class="ics-nav-button" id="ics-next-button'.$cuid.'" href="javascript:void(0);">'.__(' ','WPICSImporter').' &raquo;</a></div>';
		...
		static function showCalendar($options, $selected_date=NULL, $eventid='', $eventdate='', $cuid=''){
			...
			//	print_r($today['month']);	// will print 'January'... could be used to display month specific Picture..
			// DONE: do not hardcode PATH :)
			$PicDirUrl = plugin_dir_url(__FILE__);
			//	echo $PicDirUrl."\n";
			//	Note: all Picture files must have suffix '.jpg' ! (for now...)
			$MonthPictureUrl = $PicDirUrl.'calendars/'.$today['month'].'.jpg';
			//	echo $MonthPictureUrl;
			...
			$events = ICalEvents::get_events($options, $icsFile, $calArray[0], end($calArray)+(24*3600)+1, NULL);
			...
			$calContent = '';
			// DONE: check if $MonthPictureUrl (file) exists !
			if (@getimagesize($MonthPictureUrl))	{
				$calContent = '<img src="'.$MonthPictureUrl.'">';
			}
			...
			for ($i=0;$i<$lineWeeks;$i++){
				...
				for ($j=0;$j<7;$j++){
					...
					if ($x < (count($calArray) - 1)) $x++; #increase to the next item in the array., but not over limits 15.11.18
...
/*------------>*/	$CAL_NUM_DISPLAY = $options['cal_events_per_day'];
					...
					if(isset($calEvents) && array_key_exists($eventsKey,$calEvents)) {
						...
						if(count($calEvents[$eventsKey])>$CAL_NUM_DISPLAY) {
							...
							foreach($calEvents[$eventsKey] as $event) {
								...
								//
								/*
								$bgcol = $event['bgColor'];	// funktioniert so nicht :)
								$calContent .= '<div class="ics-calendar-event" style="background-color:'.$bgcol.';" icstag="'.$eventUID.$eventsKey.$cuid.'">';
								*/
						...
						foreach($calEvents[$eventsKey] as $key=>$event) {
							if(!isset($event['SkipBox'])) {
								...
								// 06-02-18 josswern:
								if(!empty($event['URL'])) $calContent .= __('URL:','WPICSImporter').'<a href= ' . $event['URL'] . '>' . $event['URL'] . '</a><br />';
						...
						$Rows=count($calEvents); $columns=(count($calEvents,1)-count($calEvents))/count($calEvents) - 1;
						
						$f = 0;
						//for($f=0; $f<$CAL_NUM_DISPLAY; $f++) {	// old, produces index notice
						foreach ($calEvents[$eventsKey] as $fevent) {	// DONE: fix php Notice Undefined offset: 1 here 25.11.18
							if ($f < $columns)	{	// housekeeping 15.11.18
								if($fevent['Summary']!='') {
									$eventUID = 'ics_'.md5($fevent['UID']);
									// DONE: jedem Kalendereintrag die richtige HintergrundFarbe zuordnen ! 20.01.18
									$bgcol = $fevent['bgColor'];	// funktioniert !! 20.01.18
									$calContent .= '<div class="ics-calendar-event" style="background-color:'.$bgcol.';" icstag="'.$eventUID.$eventsKey.$cuid.'">';
									// TODO: set ics_bgcolors via Admin Page 
									// alt: $calContent .= '<div class="ics-calendar-event" icstag="'.$eventUID.$eventsKey.$cuid.'">';
			...
			// DONE: global $bgcol definition:
			$bgcol = unserialize($options['ics_bgcolors']);	// aber so :) - bisher aber nur auf srv, NICHT auf live-site !! s.u.
			// nach ICS Admin Options save im Backend gehts aber auch live !
			$f = 0;
			$calContent .= __('Legend:', 'WPICSImporter');
			$calContent .= '<ul class="ics-calendar-legend" style="list-style-type:none;">';
			foreach ($icsFile as $file)	{
				$calFileName = basename($file);
				$calFileName = pathinfo($calFileName, PATHINFO_FILENAME);	// strip suffix
				$calContent .= '<li style="background-color:'.$bgcol[$f].'; width:25%; font-size: 0.75em; padding-left:1em; margin-bottom: 0.2em;">'.$calFileName.'</li>';
				$f++;
			}
			$calContent .= '</ul>';
			return $calContent;
		...
		function eventBoxArray($array, $options, $uid) {
			...
			foreach($array as $event) {
				...
				$bg = 'yellow';	// debug 
				$eventContent .= '<div class="ics-calendar-event" style="background-color:"'.$bg.'";" icstag="'.$eventUID.$uid.'">';
		...
		static function getCalendarArray($today, $options) {
			$calEvents = array();
			$buildDay = 1;
			...
			if ($buildDay <= $lastDay['mday']){
				for ($i=0; $i<7;$i++){
					/* this makes no sense :) 15.11.18, $actday is undefined, anyway
