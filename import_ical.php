...
function ics_import_parse ( $cal_file, $uid = NULL ) {
	...
	if (!$fd=@fopen($cal_file,"r")) {
		...
		$data = '';
		$line = 0;	// prevent $line undefined 15.11.18
		// while (!feof($fd) && !$error) {
		while (!feof($fd)) {
			$line++;
			$data .= fgets($fd, 4096);
		...
		$error = false;
		// $line = 0;	// should this begin with 0 again ? (see above 15.11.18)
		//$event = '';	// original
		$event = array('RAW' => '', 'summary' => '', 'actday' => '', 'recurrence-id'=>'','dtstart' => '', 'dtend' => '','duration'=>'','eventdate' => '', 'location' => '', 'url' => '', 'description' => '', 'class' => '', 'uid' => '', 'rrule' => '', 'status' => '');	// php 7.2 fix 14.11.18
		...
		for ( $n = 0; $n < count ( $lines ) && ! $error; $n++ ) {
			...
			// check $y value (housekeeping) 15.11.18
			while (($y < count($lines)) && (preg_match("/^\s/",$lines[$y]))) {
				$buff .= trim($lines[$y]);
				$lines[$y] = NULL;
				$y++;
			}
			...
			if ($state == "VEVENT") {
			// $event['RAW'][] = $buff;	// original
			$event['RAW'] = $buff;	// php7.2 fix ?
...
$plus_y = '0') {
	...
	//20090317T184000Z
	$y = intval(substr($vdate, 0, 4)) + intval($plus_y);
...
function format_ical(&$event) {
	...
	$fevent['Untimed'] = 0;	// default 25.11.18 (prevent index Untimed undefined Notice...)
