...
//	neu 27.12.17 josswern:
define('ICSFOLDER', dirname(plugin_basename(__FILE__)));
define('ICSCALENDAR_CACHEPATH', ABSPATH.'/wp-content/plugins/' . ICSFOLDER.'/cache');
...
if (! class_exists('ICalEvents')) {
	class ICalEvents {
		...
		static function display_events($url, $gmt_start, $gmt_end, $limit, $start=-1, $uuid=NULL) {
			...
			foreach ($events as $event) {
				$output .= '<li style="list-style:none;">';	// BgColor Calendar Spec. 20.01.18
				...
				if (!empty($event['Summary'])) {
					$output .= '<i>';
					// $output .= 'BgCol'.$bgcolor;	// debug, disabeled 28.12.18
		...
		static function display_one_event($url, $UID) {
			$icsOptions = get_option(ADMIN_OPTIONS_NAME);
		...
		static function custom_display_events($options, $return_count = false) {
			...
			$events = ICalEvents::get_events($options, $icsFile, $gmt_start, $gmt_end, ($options['event_limit'] * (int)$options['events_page']) );
			if (!is_array($events)) return $events;
		 ...
		 */
		static function get_event($url, $uid, $date='') {
			...
			foreach($url as $u) {
				...
				foreach($events as $k=>$na) {
					...
					$events[$k]['bgColor'] = $bgcol[$calID - 1];	// BgColor Calendar Spec. 20.01.18
				}
		 ...
		 * print_r.
		 */
		static function get_events($options, $url, $gmt_start = null, $gmt_end = null, $limit = null) {
			if($url=='') return "<i>".__('There is no calendar set. Please review the calendar settings.','WPICSImporter')."</i>";
			
			// $bgcol = array('LightSalmon','IndianRed','LightSkyBlue','GoldenRod','Beige');	// Default
			// DONE: get $bgcol from $options !
			$bgcol = unserialize($options['ics_bgcolors']);
			// ugly hack for live site 21.01.18:
		...
		static function update_cache($url) {
			$file = ICalEvents::get_cache_file($url);
			$data = wp_remote_fopen($url);
		 ...
		 */
		static function get_cache_file($url) {
		 ...
		 */
		static function get_cache_path() {
			//	default, cache Path direkt unter wp-content -> Müll :)
			//	$cache_path = rtrim(ABSPATH . 'wp-content/ics-importer-cache', '/').'/';
			//	neu 27.12.17 josswern:
			$cache_path = ICSCALENDAR_CACHEPATH.'/';
			//	echo "<br>Cache:".$cache_path;
		 ...
		 */
		static function get_cache_filename($url) {
		 ...
		 * events.
		 */
		static function constrain($events, $gmt_start = null, $gmt_end = null, $limit = null) {
		...
		static function sort_by_key($data, $key) {
			// Reverse sort
			// DONE: get rid of DEPRECATED create_function()	php7.2
			// $compare = create_function('$a, $b', 'if ($a["' . $key . '"] == $b["' . $key . '"]) { return 0; } else { return ($a["' . $key . '"] < $b["' . $key . '"]) ? -1 : 1; }');
			$compare = function($a, $b) use ($key) { if ($a[$key] == $b[$key]) { return 0; } else { return ($a[$key] < $b[$key]) ? -1 : 1; }};
			usort($data, $compare);
			return $data;
		}

		/*
		 * Return true if the specified event falls between the given
		 * start and end times.
		 */
		static function falls_between($event, $gmt_start, $gmt_end) {
			$falls_between = false;
		 ...
		 */
		static function collapse_repeats($events, $gmt_start, $gmt_end, $limit) {
			$repeats = array();
		 ...
		 */
		static function get_repeats_between($event, $gmt_start, $gmt_end, $limit, $events) {
		 ...
		 */
		static function get_repeat_days($repeats) {
		 ...
		 */
		static function get_repeat($event, $interval, $count, $repeat_day, $repeat_week='') {
			$repeat = ICalEvents::get_simple_repeat($event, $interval, $count);
		 ...
		 * future.
		 */
		static function get_simple_repeat($event, $interval, $count) {
			$duration = 0;
		 ...
		 * return false.
		 */
		static function after_rrule_end_time($repeat, $rrule) {
		 ...
		 */
		static function is_duplicate($event1, $event2) {
		 ...
		 */
		static function format_date_range($gmt_start, $gmt_end, $untimed, $date_format, $time_format, $separator = ' &ndash; ') {
			$output = '';
		 ...
		 */
		static function format_date_range_part($gmt, $untimed, $only_use_time, $date_format, $time_format) {
		 ...
		 */
		static function is_all_day($gmt1, $gmt2) {
		 ...
		 */
		static function is_same_day($gmt1, $gmt2) {
		...
		static function fdate($format, $gmt) {
			$icsOptions = get_option(ADMIN_OPTIONS_NAME);
